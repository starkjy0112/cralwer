...
LIST_URL = f"{BASE_URL}/saeol/gosi.es"
PAGE_SIZE = 10
ORGANIZATION_NAME = "광양시청"


class GwangyangCrawler:
    """광양시청 고시/공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        # 총 건수 추정
        first_row = table.select_one("tbody tr td:first-child")
        if first_row:
            try:
                total_count = int(first_row.get_text(strip=True))
            except ValueError:
                pass
        ...
        if not total_count:
            total_count = page * PAGE_SIZE if items else 0
