...
LIST_URL = f"{BASE_URL}/notice/list.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "강남구청"
MID = "ID05_040201"
...
class GangnamCrawler:
    ...
    def _fetch_page(self, keyword, page):
        ...
        # 총 건수: table row의 첫 번째 번호
        total_count = 0

        items = []
        table = soup.find("table", class_="table-hover") or soup.find("table", class_="table")
        if not table:
        ...
        # 페이지네이션에서 총 페이지 수 추정
        if not total_count:
            paging = soup.find("ul", class_="pagination") or soup.find("div", class_="pagination")
            if paging:
                max_page = 1
