...
class YUCCrawler:
    ...
    def _fetch(self, m_no, page):
        url = f"{self.BASE_URL}/www/brd/m_{m_no}/list.do?page={page}"
    ...
    def _get_total(self, html):
        m = re.search(r'전체\s*<strong>\s*([\d,]+)', html)
        return int(m.group(1).replace(',', '')) if m else 0

    def _parse_rows(self, soup, board_name, m_no):
        results = []
        if not soup:
            return results
        for tr in soup.select('tbody tr'):
            ...
            if len(tds) < 2:
                continue
            a = tr.select_one('td.al a, a[href*="view.do"]')
            if not a:
                continue
            href = a.get('href', '') or ''
            ...
            if not m:
                continue
            seq = m.group(1)
            url = f"{self.BASE_URL}/www/brd/m_{m_no}/view.do?seq={seq}"
            ...
            results.append({
                ...
                'organization': board_name, 'number': seq,
            })
        return results

    def _crawl_board(self, board_tuple):
        m_no, board_name = board_tuple
        results = []
        seen = set()
        first_soup, first_html = self._fetch(m_no, 1)
        if not first_soup:
            return results
        total = self._get_total(first_html)
        for it in self._parse_rows(first_soup, board_name, m_no):
        ...
        for page in range(2, last_p + 1):
            ...
            soup, _ = self._fetch(m_no, page)
            rows = self._parse_rows(soup, board_name, m_no)
            for it in rows:
                if it['url'] not in seen:
                    seen.add(it['url'])
                    results.append(it)
    ...
    def search(self, keyword='', max_pages=999, start_date=None, end_date=None):
        ...
        print(f"[용인도시공사] 완료: 총 {len(items)}건", flush=True)
        return items
