...
"""
광진구청 고시공고 크롤러
https://www.gwangjin.go.kr/portal/bbs/B0000378/list.do?menuNo=200192
GET 기반, 10건/페이지
"""
import math
import re
...
LIST_URL = f"{BASE_URL}/portal/bbs/B0000378/list.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "광진구청"
MENU_NO = "200192"


class GwangjinCrawler:
    """광진구청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        for t in soup.find_all("table"):
            ...
            if len(rows) > 3:
                ...
                if any("번호" in h for h in ths) or any("공고" in h for h in ths):
                    table = t
                    break
        ...
        if table:
            ...
            for tr in rows:
                ...
                if len(tds) < 5:
                    continue
                # 번호, 공고번호, 진행사항, 공고명, 담당부서, 공고시작일, 공고종료일
                number = tds[0].get_text(strip=True)
                if not total_count and number.replace(",", "").strip().isdigit():
                    total_count = int(number.replace(",", "").strip())
        ...
        # 페이지네이션 추정
        if not total_count:
            paging = soup.find("div", class_="paginationSet")
            if paging:
                max_page = 1
