...
"""
안양시청 입찰공고 크롤러
https://www.anyang.go.kr/main/emwsWebList.do?key=4103&searchGosiSe=02
GET 기반, 10건/페이지
"""
import math
import re
...
class AnyangBidCrawler:
    ...
    def _fetch_page(self, keyword, page):
        ...
        items = []
        # Table: class="p-table simple small"
        table = soup.find("table", class_="p-table")
        if not table:
            for t in soup.find_all("table"):
                caption = t.find("caption")
                if caption and "입찰" in caption.text:
                    table = t
                    break
        if not table:
            for t in soup.find_all("table"):
