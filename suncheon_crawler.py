...
GET 기반, table 7열 (번호/분류/제목/부서/등록일/게재기간/조회), pageIndex 파라미터
"""
import math
import re
import requests
...
LIST_PATH = "/kr/news/0004/0005/0001/"
LIST_URL = f"{BASE_URL}{LIST_PATH}"
PAGE_SIZE = 10
ORGANIZATION_NAME = "순천시청"


class SuncheonCrawler:
    """순천시청 고시/공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        for row in rows:
            ...
            category = cells[1].get_text(strip=True) if len(cells) >= 7 else ""
            title_cell = cells[2] if len(cells) >= 7 else cells[1]
            dept = cells[3].get_text(strip=True) if len(cells) >= 7 else cells[2].get_text(strip=True)
            date = cells[4].get_text(strip=True) if len(cells) >= 7 else cells[3].get_text(strip=True)
