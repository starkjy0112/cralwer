...
"""
구례군청 고시/공고 크롤러
https://www.gurye.go.kr/board/GosiList.do?not_ancmt_se_code=01,04,06,07&menuNo=115004002001
...
class GuryeCrawler:
    ...
    def _fetch_page(self, keyword, page):
        ...
        for row in rows:
            ...
            detail_url = LIST_URL + f"?menuNo=115004002001"
            if link:
                href = link.get("href", "") or link.get("onclick", "")
                m_id = re.search(r"searchDetail\('(\d+)'\)", href)
                if m_id:
                    detail_url = f"{BASE_URL}/board/GosiView.do?not_ancmt_mgt_no={m_id.group(1)}&menuNo=115004002001"
