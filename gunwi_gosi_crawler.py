...
"""
군위군청 고시공고 크롤러
https://www.gunwi.go.kr/ko/page.do?mnu_uid=666&boardType=notice
GET 기반, table.tbl_board, 10건/페이지
컬럼: 번호, 제목, 작성자, 작성일, 파일, 조회
"""
import math
import re
import requests
...
BASE_URL = "https://www.gunwi.go.kr"
LIST_URL = f"{BASE_URL}/ko/page.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "군위군청"


class GunwiGosiCrawler:
    """군위군청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        total_count = 0
        items = []
        table = soup.find("table", class_="tbl_board") or soup.find("table")
        if not table:
        ...
        for tr in rows:
            ...
            if href.startswith("?"):
                ...
                detail_url = f"{LIST_URL}?mnu_uid=666"
