...
BASE_URL = "https://www.jecheon.go.kr"
LIST_URL = f"{BASE_URL}/www/selectBbsNttList.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "제천시청"


class JecheonGosiCrawler:
    """제천시청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        for tr in tbody.find_all("tr"):
            ...
            if len(tds) < 5:
                continue
            # Columns: 번호, 제목, 첨부, 담당부서, 등록일, 조회수, 고시공고번호
            ...
            # gosi_no is in last column
            gosi_no = tds[-1].get_text(strip=True) if len(tds) > 6 else ""
            if gosi_no:
                title = f"[{gosi_no}] {title}"
