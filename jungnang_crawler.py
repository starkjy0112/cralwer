...
GET 기반, portal BBS 시스템, inc_head 테이블, 10건/페이지
"""
import math
import re
...
LIST_URL = f"{BASE_URL}/portal/bbs/list/B0000117.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "중랑구청"


class JungnangCrawler:
    """중랑구청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        if hasattr(self, '_edate') and self._edate:
            params["edate"] = self._edate
        ...
        items = []
        table = soup.select_one("table.inc_head, table")
        if not table:
        ...
        for row in rows:
            ...
            dept = cells[2].get_text(strip=True) if len(cells) > 2 else ORGANIZATION_NAME
            # cells[3] = 파일
    ...
    def search(self, keyword="", max_pages=10, start_date=None, end_date=None):
        if start_date:
            self._sdate = start_date
        if end_date:
            self._edate = end_date
