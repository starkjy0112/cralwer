...
"""
단양군청 고시공고 크롤러
https://www.danyang.go.kr/dy21/976
실제 데이터는 eminwon.danyang.go.kr OfrAction (POST 기반, 구형 eminwon)
"""
import math
import re
import requests
...
JSP_URL = "https://eminwon.danyang.go.kr/emwp/jsp/ofr/OfrNotAncmtL.jsp"
EMINWON_URL = "https://eminwon.danyang.go.kr/emwp/gov/mogaha/ntis/web/ofr/action/OfrAction.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "단양군청"


class DanyangGosiCrawler:
    """단양군청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        # Parse total from "전체게시물:XXXXX개"
        total_m = re.search(r"전체게시물:(\d+)개", resp.text)
        if total_m:
            total_count = int(total_m.group(1))

        # Old eminwon uses nested tables. Data rows have searchDetail onclick.
        ...
        # Each item typically has: number, gosi_no, title, dept, date, (period), views
        for mgt_no, texts in seen_ids.items():
            texts = [t for t in texts if t]
            if len(texts) < 4:
                continue

            number = texts[0] if texts[0].isdigit() else ""
            gosi_no = ""
            title_text = ""
            dept = ""
            date = ""
            ...
            for t in texts:
                if re.match(r"^\d+$", t) and not number:
                    number = t
                elif re.match(r".+제\d{4}-\d+호", t) or "공고" in t and "제" in t:
                    gosi_no = t
                elif re.match(r"\d{4}-\d{2}-\d{2}", t):
                    date = t
                elif len(t) > 10 and not title_text:
                    title_text = t
                elif len(t) < 10 and not dept and not t.isdigit() and not re.match(r"\d{4}", t):
                    dept = t

            if not title_text and len(texts) >= 3:
                title_text = texts[2] if len(texts[2]) > len(texts[1]) else texts[1]
            if not date:
                for t in texts:
            ...
            if not dept:
                for t in texts:
                    if "과" in t or "실" in t or "소" in t or "원" in t or "동" in t or "면" in t:
                        if len(t) < 15:
                            dept = t
                            break
