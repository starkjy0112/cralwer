...
POST 기반, table.bod_list, 10건/페이지
컬럼: 번호, 공고번호, 제목, 담당자 연락처, 담당부서, 등록일
"""
import math
import re
import requests
...
BASE_URL = "https://www.gbmg.go.kr"
LIST_URL = f"{BASE_URL}/portal/saeol/gosi/list.do"
PAGE_SIZE = 10
ORGANIZATION_NAME = "문경시청"


class MungyeongGosiCrawler:
    """문경시청 고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        resp = self.session.post(
            f"{LIST_URL}?mId=0301060000", data=data, timeout=15
        )
        resp.encoding = "utf-8"
        ...
        for tr in rows:
            ...
            if a_tag:
                ...
                if data_action:
                    ...
                    detail_url = f"{LIST_URL}?mId=0301060000"
            else:
                title = tds[2].get_text(strip=True)
                detail_url = f"{LIST_URL}?mId=0301060000"

            # td[3]=담당자연락처, td[4]=담당부서, td[5]=등록일
