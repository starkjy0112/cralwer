# -*- coding: utf-8 -*-
"""
과천도시공사 (gcuc) - xlsx 37행
12개 게시판 합산. page= 페이징, 빈 페이지에서 종료.
"""
import re
import time
import requests
...
class GCUCCrawler:
    ...
    def _parse_rows(self, soup, board_name, code):
        ...
        for tr in soup.select('tbody tr'):
            ...
            if len(tds) < 2:
                continue
            # 두 가지 URL 형식 지원:
            # (1) action-value=<hash>   (일반 게시판)
            # (2) action=detail&recruit_srl=<num>   (채용공고)
            a = tr.select_one('a[href*="action-value"], a[href*="recruit_srl"], a[href*="action=detail"]')
            if not a:
                continue
            href = a.get('href', '') or ''
            # ID 추출
            m_hash = re.search(r'action-value=([a-f0-9]+)', href)
            m_srl = re.search(r'recruit_srl=(\d+)', href)
            if m_hash:
                post_id = m_hash.group(1)
                url = f"{self.BASE_URL}/fmcs/{code}?action=read&action-value={post_id}"
                number = post_id[:8]
            elif m_srl:
                post_id = m_srl.group(1)
                url = f"{self.BASE_URL}/fmcs/{code}?action=detail&recruit_srl={post_id}"
                number = post_id
            else:
                continue
    ...
    def search(self, keyword='', max_pages=999, start_date=None, end_date=None):
        ...
        print(f"[과천도시공사] 완료: 총 {len(items)}건", flush=True)
        return items
