...
GET 기반, 20건/페이지, table.skinTb
goPage(articleSeq) onclick 패턴
"""
import math
import re
...
LIST_URL = f"{BASE_URL}/portal/adm/bulletin/notify"
PAGE_SIZE = 20
ORGANIZATION_NAME = "인제군청"


class InjeGosiCrawler:
    """인제군청 일반고시공고 크롤러"""
    ...
    def _fetch_page(self, keyword, page):
        ...
        if m:
            total_count = int(m.group(1).replace(",", "").strip())
        ...
        for tr in rows:
            ...
            # onclick="goPage(248474); return false;"
            onclick = a_tag.get("onclick", "")
            seq_match = re.search(r'goPage\((\d+)\)', onclick)
            if seq_match:
                detail_url = f"{LIST_URL}?articleSeq={seq_match.group(1)}"
            else:
                href = a_tag.get("href", "")
                if href.startswith("/"):
