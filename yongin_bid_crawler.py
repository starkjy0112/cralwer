...
"""
용인특례시청 입찰공고 크롤러
http://www.yongin.go.kr/home/yiNw/yiNwInfo/yiNwInfo01/yiNwInfo01_01.jsp
...
BASE_URL = "https://eminwon.yongin.go.kr"
...
class YonginBidCrawler:
    ...
    def _fetch_page(self, keyword, page):
        ...
        # Find the data table (not search table)
        tables = soup.find_all("table")
        table = None
        for t in tables:
            if t.select("tbody tr td"):
                rows_check = t.select("tbody tr")
                if rows_check and len(rows_check[0].find_all("td")) >= 5:
                    table = t
                    break
        ...
        for row in rows:
            ...
            if link:
                ...
                if m:
                    mgt_no = m.group(1)
                    detail_url = (
                        f"{DETAIL_BASE}?not_ancmt_se_code=02"
    ...
    def search(self, keyword="", max_pages=10, start_date=None, end_date=None):
        ...
        print(f"[용인특례시청 입찰공고] 완료: 총 {len(all_items)}건")
        return all_items
