...
"""
중구청 고시공고 크롤러
https://www.junggu.seoul.kr/content.do?cmsid=14232
GET 기반, searchForm, euc-kr 인코딩 페이지
"""
import math
import re
...
LIST_URL = f"{BASE_URL}/content.do"
CMS_ID = "14232"
PAGE_SIZE = 10
ORGANIZATION_NAME = "중구청"
...
class JungguCrawler:
    ...
    def _fetch_page(self, keyword, page):
        ...
        # Try euc-kr decoding since it's a Seoul district site
        try:
        ...
        m = re.search(r'총\s*(\d[\d,]*)\s*건', page_text)
        if m:
            total_count = int(m.group(1).replace(",", ""))
        if not total_count:
            m2 = re.search(r'전체\s*(\d[\d,]*)', page_text)
            if m2:
